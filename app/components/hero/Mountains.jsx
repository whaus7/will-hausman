...
export default function Mountains() {
  ...
  // Track scroll position for vertical parallax
  useEffect(() => {
    let ticking = false
  ...
  const scrollPos = useCallback((pathname, layer) => {
    let baseScroll = 20 // More reasonable movement distance
    const scrollDistance = layer * 20 // More reasonable movement distance
  ...
  // Calculate vertical parallax offset for each layer
  const getVerticalParallax = useCallback(
    (layer) => {
      // Different layers move at different speeds
      // Layer 1 (furthest back) moves slowest
      // Trees (layer 4) move fastest to eclipse the background
      const speeds = {
        1: 0.05, // Mountain Layer 1 - slowest (furthest back)
        2: 0.15, // Mountain Layer 2
        3: 0.25, // Mountain Layer 3
        4: 0.4, // Trees and hills - fastest (closest to viewer)
      }

      const speed = speeds[layer] || 0.05
      return -scrollY * speed // Negative value reverses the scroll direction
    },
    [scrollY]
  )
    ...
    >
      ...
      <path
        ...
        style={{
          ...
          transform: `translateX(${scrollPos(pathname, 1)}) translateY(${getVerticalParallax(1)}px) scale(3.0)`,
        }}
      ...
      <path
        ...
        style={{
          ...
          transition: 'transform 0.8s ease-out',
          transform: `translateX(${scrollPos(pathname, 2)}) translateY(${getVerticalParallax(2)}px) scale(3.0)`,
        }}
      ...
      <path
        ...
        style={{
          ...
          transition: 'transform 0.8s ease-out',
          transform: `translateX(${scrollPos(pathname, 3)}) translateY(${getVerticalParallax(3)}px) scale(3.0)`,
        }}
      ...
      <g
        style={{
          transition: 'transform 0.8s ease-out',
          transform: `translateX(${scrollPos(pathname, 4)}) translateY(${getVerticalParallax(4)}px)`,
        }}
      >
        {treeElements}
      </g>
      ...
      <path
        ...
        style={{
          transition: 'transform 0.8s ease-out',
          transform: `translateX(${scrollPos(pathname, 4)}) translateY(${getVerticalParallax(4)}px) scale(3.0)`,
          //scale: 3,
          fill: darkmode
